...
for ($i=0; $i -lt $args.Length; $i++)
{
    ...
    if ($key -eq "-h" -Or $key -eq "--help")
    {
        ...
        echo ""
        echo "Options:"
        echo "`t-c, --config`t`t`t`t`tPath to a LitGit configuration file: see https://github.com/jvbsl/LitGit"
        echo "`t-s, --searchpath`t`t`t`tPath to a directory in which to search for *.template files. Default is directory of script."
        echo "`t-d, --destination-dir`t`t`t`tThe output directory to which the templated files are written, if not specified otherwise by (-o | --outputs)."
